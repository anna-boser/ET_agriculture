...
tidy <- function(data){
  ET <- data[,12:14]
  data <- data[,1:11]
  clean <- pivot_longer(data, cols = paste0("PET", 2:4), names_to = "monthgroup", names_prefix = "PET", values_to = "PET")
  ET <- pivot_longer(ET, cols = paste0("ET", 2:4), names_to = "monthgroup", names_prefix = "ET", values_to = "ET")
  clean$ET <- ET$ET
